...
BEGIN
    ...
    BEGIN
        ...
        IF ISNULL(@exists,0) = 0
        BEGIN
            RAISERROR('No se logr� descargar el JSON (ni con curl ni con certutil).', 16, 1);
            RETURN;
        END
    END
    ...
    BEGIN
        RAISERROR('El archivo se descarg� pero vino vac�o o no se pudo leer como UTF-8.', 16, 1);
        RETURN;
    END
    ...
    BEGIN
        ...
        RAISERROR('El contenido no parece JSON v�lido. Inicio: %s', 16, 1, @snippet);
        RETURN;
    END
...
/* ============================================================
   4. FUNCI�N: OBTENER 5� D�A H�BIL DEL MES
============================================================ */
...
BEGIN
    ...
    -- Cargar feriados si es enero y no est�n cargados
    ...
    -- Generar expensas solo si hoy es el 5� d�a h�bil
    IF @Hoy = @QuintoDiaHabil
    BEGIN
        PRINT 'Generando expensas - Quinto d�a h�bil: ' + CONVERT(VARCHAR, @Hoy, 103);
    ...
    END
    ELSE
        PRINT 'Hoy no es quinto d�a h�bil. Pr�ximo: ' + CONVERT(VARCHAR, @QuintoDiaHabil, 103);
END;
GO

/* ============================================================
   6. PROCEDIMIENTO: SIMULAR ENV�O DE EXPENSAS
============================================================ */
...
/* ============================================================
   8. CONFIGURAR JOBS EN SQL SERVER AGENT (SI EST� DISPONIBLE)
============================================================ */
USE msdb;
GO
...
/* ===== Job 1: Verificaci�n diaria de expensas ===== */
EXEC dbo.sp_add_job
    @job_name = N'GenerarExpensas_VerificacionDiaria',
    @enabled = 1,
    @description = N'Verifica diariamente si es el quinto d�a h�bil para generar expensas.';
