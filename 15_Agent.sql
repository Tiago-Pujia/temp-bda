...
BEGIN
    ...
    BEGIN
        ...
        IF ISNULL(@exists,0) = 0
        BEGIN
            RAISERROR('No se logrï¿½ descargar el JSON (ni con curl ni con certutil).', 16, 1);
            RETURN;
        END
    END
    ...
    BEGIN
        RAISERROR('El archivo se descargï¿½ pero vino vacï¿½o o no se pudo leer como UTF-8.', 16, 1);
        RETURN;
    END
    ...
    BEGIN
        ...
        RAISERROR('El contenido no parece JSON vï¿½lido. Inicio: %s', 16, 1, @snippet);
        RETURN;
    END
...
/* ============================================================
   4. FUNCIï¿½N: OBTENER 5ï¿½ Dï¿½A Hï¿½BIL DEL MES
============================================================ */
...
BEGIN
    ...
    -- Cargar feriados si es enero y no estï¿½n cargados
    ...
    -- Generar expensas solo si hoy es el 5ï¿½ dï¿½a hï¿½bil
    IF @Hoy = @QuintoDiaHabil
    BEGIN
        PRINT 'Generando expensas - Quinto dï¿½a hï¿½bil: ' + CONVERT(VARCHAR, @Hoy, 103);
    ...
    END
    ELSE
        PRINT 'Hoy no es quinto dï¿½a hï¿½bil. Prï¿½ximo: ' + CONVERT(VARCHAR, @QuintoDiaHabil, 103);
END;
GO

/* ============================================================
   6. PROCEDIMIENTO: SIMULAR ENVï¿½O DE EXPENSAS
============================================================ */
...
/* ============================================================
   8. CONFIGURAR JOBS EN SQL SERVER AGENT (SI ESTï¿½ DISPONIBLE)
============================================================ */
USE msdb;
GO
...
/* ===== Job 1: Verificaciï¿½n diaria de expensas ===== */
EXEC dbo.sp_add_job
    @job_name = N'GenerarExpensas_VerificacionDiaria',
    @enabled = 1,
    @description = N'Verifica diariamente si es el quinto dï¿½a hï¿½bil para generar expensas.';
...
SELECT 
    @@VERSION AS Version,
    SERVERPROPERTY('Edition') AS Edition,
>>>>>>> 4221ede1b7b4b877976ccb41761ccde5820118b0
